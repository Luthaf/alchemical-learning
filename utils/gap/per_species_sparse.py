...
class PerSpeciesSparseGapModel(torch.nn.Module):
    def __init__(
        self,
        ...
        if optimize_weights:
            self.weights = torch.nn.ParameterDict(
                {str(s): torch.nn.Parameter(w.detach()) for s, w in weights.items()}
            )
        else:
...
def train_per_species_sparse_gap_model(
    ...
    support_points = _select_support_points(power_spectrum, all_species, n_support)
    ...
    return PerSpeciesSparseGapModel(kernels, weights_per_species, optimizable_weights)
...
def _select_support_points(power_spectrum, all_species, n_support):
    ...
    for species, n_to_select in n_support.items():
        ...
        support_points[species] = X[fps.selected_idx_]
