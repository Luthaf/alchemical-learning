...
from utils.soap import CompositionFeatures, PowerSpectrum
...
np.save(
    f"{prefix}-hypers.npy",
    dict(powerspectrum=HYPERS_SMALL, radialspectrum=HYPERS_RADIAL),
)
...
if do_gradients is True:
    ...
    train_dataset_grad = AtomisticDataset(
        train_frames,
        all_species,
...
)  # , line_search_fn="strong_wolfe", history_size=128)

all_losses = []
all_tests = []

filename = (
    f"{prefix}-{model.__class__.__name__}-{N_PSEUDO_SPECIES}-mixed-{n_train}-train"
)
if model.optimizable_weights:
...
for epoch in range(300):
    ...
    def single_step():
        ...
        if high_mem:
            predicted, _ = model(
                composition, radial_spectrum, spherical_expansions, forward_forces=False
            )
        ...
        print(
            loss.item(), torch.linalg.norm(model.composition_model.weights.grad).item()
        )
        return loss
...
with torch.no_grad():
    ...
    for (
        ...
        predicted_e, _ = model(
            tcomposition, tradial_spectrum, tspherical_expansions, forward_forces=False
        )
